resource "azurerm_container_registry" "this" {
  ...
  lifecycle {
    precondition {
      condition     = var.zone_redundancy_enabled && var.container_registry_sku == "Premium" || !var.zone_redundancy_enabled
      error_message = "The Premium SKU is required if zone redundancy is enabled."
    }
    precondition {
      condition     = var.network_rule_set != null && var.container_registry_sku == "Premium" || var.network_rule_set == null
      error_message = "The Premium SKU is required if a network rule set is defined."
    }
    precondition {
      condition     = var.customer_managed_key != null && var.container_registry_sku == "Premium" || var.customer_managed_key == null
...
resource "azurerm_management_lock" "this" {
  ...
  lock_level = var.container_registry_lock.kind
  name       = coalesce(var.container_registry_lock.name, "lock-${var.container_registry_name}")
  scope      = azurerm_container_registry.this.id
  notes      = var.container_registry_lock.kind == "CanNotDelete" ? "Cannot delete the resource or its child resources." : "Cannot delete or modify the resource or its child resources."
}
